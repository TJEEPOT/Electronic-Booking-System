...
public class SampleBase<T> {
    ...
    protected <C> C load(String jsonFile, Class<C> clazz) throws IOException {
        ...
        try {
//            br = new BufferedReader(new FileReader(new File(getClass().getClassLoader().getResource(jsonFile).getFile())));
//            URL resourceURL = this.getClass().getClassLoader().getResource(jsonFile);
//            if(resourceURL == null)
//                throw new FileNotFoundException(jsonFile+" not found");

            String name = new File(".").getCanonicalPath();
            File file = new File(name, "/"+ "invoice_create.json");
